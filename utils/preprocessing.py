...
var_attributes =  pd.read_excel('data/raw/DIAS Attributes - Values 2017.xlsx',
                               header = 1,
                                 usecols = 'B:F',
                                 dtype = str,
                                 sheet_name = 'Tabelle1_fixed')

CENSUS_VAR_TYPES =  var_attributes[['Attribute','Type']].dropna()

CENSUS_VAR_TYPES.loc[CENSUS_VAR_TYPES['Attribute'] == 'ANZ_HH_TITEL','Type'] = 'binary'

def generate_nan_map(var_attributes = var_attributes):
...
def reencode_d19_columns(dataframe):
    ...
    d19_household_cols_list = []

    for col in dataframe.columns:

        # Skipping nominal columns
        if col in ['D19_LETZTER_KAUF_BRANCHE_RZ','D19_KONSUMTYP','D19_KUNDENTYP']:
        ...
        if col.startswith('D19'):
            ...
            if ('DATUM' in col) or ('ANZ' in col) or ('QUOTE' in col):
                ...
                d19_household_cols_list.append(col)
            
            else:

                grid_cols_list.append(col)
